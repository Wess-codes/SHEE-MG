...
export default function WorkPermitClasses() {
  ...
  return (
    <>
      <style>{`
        .wpc-item { opacity: 0; transform: translateY(20px); transition: opacity 0.5s ease, transform 0.5s ease; }
        .wpc-item.wpc-visible { opacity: 1; transform: translateY(0); }
        ...
        .wpc-card { transition: box-shadow 0.25s ease, transform 0.25s ease, border-color 0.25s ease; }
        .wpc-card:hover { box-shadow: 0 12px 32px rgba(0,0,0,0.09); transform: translateY(-3px); border-color: #ea580c !important; }
        .wpc-badge { font-family: inherit; transition: background 0.25s ease, color 0.25s ease, transform 0.25s ease; }
        .wpc-card:hover .wpc-badge-default { background: #ea580c; color: #fff; transform: scale(1.08); }
      `}</style>

      <section className="bg-[#f8f9fb] py-20 overflow-hidden">
        {/* Header */}
        <div className="max-w-6xl mx-auto px-6 sm:px-12 lg:px-20 mb-14 text-center">
          <h2
            className={`${montserrat.className} text-3xl md:text-4xl lg:text-5xl font-bold text-[#0B132B] leading-tight`}
          >
            We Offer the Following{" "}
            <span className="text-orange-500">Work Permit</span> Services
          </h2>
          <div className="w-12 h-[3px] bg-orange-500 rounded-full mx-auto mt-5" />
        </div>
        ...
        <div className="max-w-6xl mx-auto px-6 sm:px-12 lg:px-20">
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {permits.map((p, i) => (
              ...
              >
                {p.featured ? (
                  /* Featured card — light orange accent */
                  <div className="wpc-card relative bg-orange-50 border-2 border-orange-300 rounded-2xl p-6 flex flex-col h-full overflow-hidden">
                    ...
                    <p
                      className={`${openSans.className} text-sm text-gray-600 leading-relaxed mb-5 flex-1`}
                    >
                      {p.desc}
                    </p>
                    <a
                      href={`/services/work-permits#class-${p.code.toLowerCase()}`}
                      className={`${openSans.className} inline-flex items-center gap-1.5 text-xs font-semibold text-orange-500 hover:text-orange-600 transition-colors`}
                    >
